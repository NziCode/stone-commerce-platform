        ...
        @foreach($fields as $field)
            <div>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.3rem">
                    ...
                    <button type="button"
                        wire:click="mountAction('translateField', { field: '{{ $field['key'] }}', isHtml: {{ ($field['type'] ?? 'text') === 'richtext' ? 'true' : 'false' }} })"
                ...
                @foreach($activeLangs as $lang)
                    <div x-show="loc === '{{ $lang->code }}'" x-cloak>
                        @if(($field['type'] ?? 'text') === 'richtext')
                            @include('filament.pages.partials.rich-text-field', ['statePath' => "{$field['key']}.{$lang->code}"])
